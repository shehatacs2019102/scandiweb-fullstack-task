...
class ProductListingPage extends Component {
  ...
  componentDidMount() {
    fetch("http://localhost:8000/graphql", {
      ...
      body: JSON.stringify({
        query:
          "{ getProducts {tech {  id name currency_symbol amount attributes { id name value } gallery in_stock category {name} } clothes {  id name currency_symbol amount attributes { id name value } gallery in_stock category {name} }} }",
      }),
    })
      .then((res) => res.json())
      .then((data) => { const allProducts = [
        
        ...(data.data.getProducts.tech || []),
        ...(data.data.getProducts.clothes || []),
      ];
